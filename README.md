...
### v0.8.0
- **卡片系统**：4 种 Quill 卡片（图片/网址/文件链接/节点图），支持选中/多选/批量删除/双击交互
- **节点图导出 HTML**：独立 HTML 展示页，含贝塞尔连线、拖拽平移、滚轮缩放、悬停提示
- **共享 SVG 渲染引擎**：`generateNodeGraphSVG()` 统一导出与缩略图渲染管线
- **文件简介系统**：文件树右键管理简介，文件名下方显示描述文本
- **智能括号**：编辑器自动补全 `()[]{}""''`，支持配对删除
- **CSS 架构重构**：`style.css` 拆分为 5 个模块化 CSS 文件
- **编辑器模块拆分**：`editor.js` 拆分为 11 个独立模块
- **修复弹窗美化丢失**：CSS 截断、CSP 策略、缺失样式三合一修复
- **修复节点图导出**：贝塞尔曲线、拖拽视角、缩略图渲染、坐标对齐
- 多项文件树、标签系统、设置页的细节修复
